...
function rewriteHtml(response, product, url, deviceId) {
  ...
  const transformed = new HTMLRewriter()
    // Injeta <base href="/"> no início do <head> — assim os caminhos relativos
    // (catalogo.json, logo.png, etc.) resolvem para a raiz mesmo quando a página
    // está sendo servida em /produto/{id}.
    .on('head', {
      element(el) {
        el.prepend(`<base href="${url.origin}/">`, { html: true });
      },
    })
  ...
  // Impede que o Cloudflare cacheie a resposta — assim cada visita pega
  // os dados atualizados, e cada produto tem suas próprias meta tags.
...
/* Encontra o objeto JSON com "id":"X" usando text search + brace matching.
   ...
   Aceita JSON com ou sem espaços entre "id", : e o valor. */
function findProductInText(text, deviceId) {
  // Escapa o deviceId pra uso em regex
  const escapedId = deviceId.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const re = new RegExp(`"id"\\s*:\\s*"${escapedId}"`);
